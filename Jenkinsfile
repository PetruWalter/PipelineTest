...
pipeline {
    ...
    stages {
        ...
        stage('Build mit Excel erstellen') {
            steps {
                echo 'PetruTester'

                // Hier fügen Sie Ihren Code ein, um die Excel-Datei zu erstellen
                // Stellen Sie sicher, dass Ihre Dependencies in Ihrem Maven-Projekt
                // die Apache POI-Bibliothek enthalten, um auf die benötigten Klassen
                // zuzugreifen.

                // Beispielcode zum Erstellen der Excel-Datei
                script {
